...
*/
         AbstractTask t;
         stream = new FileInputStream("Supplier.clp");
         t = ZeusParser.reteKB(db,stream);
         stream.close();
         if ( t.resolve(b) )
            agent.addTask(t);
         stream = new FileInputStream("CollectorNotifier.clp");
         t = ZeusParser.reteKB(db,stream);
         stream.close();
         if ( t.resolve(b) )
            agent.addTask(t);
...
*/
         Fact f1;
         f1 = ZeusParser.fact(db,"(:type Entity :id fact38 :modifiers 0 :attributes ((unit_cost 0)(number 1)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
