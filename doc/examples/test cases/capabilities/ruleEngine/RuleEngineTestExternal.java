...
public class RuleEngineTestExternal implements ZeusExternal {
    ...
    public void exec(AgentContext context ) { 
        ...
        int testNumber = 0; 
        while (testNumber<100) { // run until stopped!
            testNumber++;
            System.out.println("Asserting "+ testNumber + " facts");  
            for (int count = 0; count < testNumber; count++ ) {
                Fact f = ont.getFact(Fact.FACT, "Entity"); 
                rdb.add(f); }    
            // pause so that we can check what has happened
           try {
                Thread.sleep(3000);
            } catch (Exception e) { 
                e.printStackTrace(); // which will never happen!
            }
