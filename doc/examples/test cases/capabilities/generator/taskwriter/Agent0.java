...
public class Agent0 {
   protected static void version() {
      System.err.println("ZeusAgent - Agent0 version: 1.1");
      System.exit(0);
   }

   protected static void usage() {
      System.err.println("Usage: java Agent0 -s <dns_file> -o <ontology_file> [-gui ViewerProg] [-e <ExternalProg>] [-r ResourceProg] [-name <AgentName>] [-debug] [-h] [-v]");
      System.exit(0);
   }
   ...
   try { 
      ...
      String name = new String ("Agent0");
      Bindings b = new Bindings("Agent0");
      ...
      for( int j = 0; j < arg.length; j++ ) {
         if ( arg[j].equals("-s") &&  ++j < arg.length )
            ...
            Core.debug = true;
            Core.setDebuggerOutputFile("Agent0.log");
         }
      ...
      if ( dns_file == null ) {
         ...
         agent = new ZeusAgent(name,ontology_file,nameservers,1,20,true,true);
...
*/
         AbstractTask t;
         t = ZeusParser.primitiveTask(db,"(:Primitive Nomboe :time (1) :cost (0))");
         if ( t.resolve(b) )
            agent.addTask(t);
         t = ZeusParser.primitiveTask(db,"(:Primitive coopie :time (1) :cost (0) :consumed_facts ((:type Entity :id var1 :modifiers 1 :attributes ((unit_cost ?var2)(number ?var3)))(:type Entity :id var4 :modifiers 1 :attributes ((unit_cost ?var5)(number ?var6)))) :produced_facts ((:type Entity :id var7 :modifiers 1 :attributes ((unit_cost ?var8)(number ?var9)))(:type Entity :id var10 :modifiers 1 :attributes ((unit_cost ?var11)(number ?var12)))))");
         if ( t.resolve(b) )
            agent.addTask(t);
         stream = new FileInputStream("Scoopbie.clp");
         t = ZeusParser.reteKB(db,stream);
         stream.close();
         if ( t.resolve(b) )
            agent.addTask(t);
         t = ZeusParser.summaryTask(db,"(:Summary lalala :is_autorun false :time (1) :cost (0) :nodes ((:name begin :is_conditional false :consumed_facts ((:group default ())) :produced_facts ((:group default ())))(:name end :is_conditional false :consumed_facts ((:group default ())) :produced_facts ((:group default ())))))");
         if ( t.resolve(b) )
            agent.addTask(t);
/*
...
*/
         Fact f1;
         f1 = ZeusParser.fact(db,"(:type Entity :id fact21 :modifiers 0 :attributes ((unit_cost 0)(number 1)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
         f1 = ZeusParser.fact(db,"(:type Entity :id fact22 :modifiers 0 :attributes ((unit_cost 0)(number 1)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
         f1 = ZeusParser.fact(db,"(:type Entity :id fact23 :modifiers 0 :attributes ((unit_cost 0)(number 1)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
         f1 = ZeusParser.fact(db,"(:type Entity :id fact24 :modifiers 0 :attributes ((unit_cost 0)(number 1)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
