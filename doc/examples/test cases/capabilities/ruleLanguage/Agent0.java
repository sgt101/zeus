...
public class Agent0 {
   protected static void version() {
      System.err.println("ZeusAgent - Agent0 version: 1.2.2");
      System.exit(0);
   }
   ...
   try { 
      ...
      if ( dns_file == null ) {
         ...
         agent = new ZeusAgent(name,ontology_file,nameservers,1,20,false,true);
...
*/
         AbstractTask t;
         stream = new FileInputStream("Task0.clp");
         t = ZeusParser.reteKB(db,stream);
         stream.close();
         if ( t.resolve(b) )
            agent.addTask(t);
         stream = new FileInputStream("Task2.clp");
         t = ZeusParser.reteKB(db,stream);
         stream.close();
         if ( t.resolve(b) )
            agent.addTask(t);
...
*/
         Fact f1;
         f1 = ZeusParser.fact(db,"(:type sub :id fact54 :modifiers 0 :attributes ((name5 ?var55)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
         f1 = ZeusParser.fact(db,"(:type super :id fact7 :modifiers 0 :attributes ((name5 ?var8)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
