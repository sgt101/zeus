...
public class LoadForecastService {
   protected static void version() {
      System.err.println("ZeusAgent - LoadForecastService version: 1.1");
      System.exit(0);
   }

   protected static void usage() {
      System.err.println("Usage: java LoadForecastService -s <dns_file> -o <ontology_file> [-gui ViewerProg] [-e <ExternalProg>] [-r ResourceProg] [-name <AgentName>] [-debug] [-h] [-v]");
      System.exit(0);
   }
   ...
   try { 
      ...
      String name = new String ("LoadForecastService");
      Bindings b = new Bindings("LoadForecastService");
      ...
      for( int j = 0; j < arg.length; j++ ) {
         if ( arg[j].equals("-s") &&  ++j < arg.length )
            ...
            Core.debug = true;
            Core.setDebuggerOutputFile("LoadForecastService.log");
         }
...
*/
         ProtocolInfo info;
         info = ZeusParser.protocolInfo(db,"(:name \"zeus.actors.graphs.ContractNetRespondent\" :type Respondent :constraints ((:fact (:type ZeusFact :id var424 :modifiers 1) :type 0 :strategy \"zeus.actors.graphs.DefaultRespondentEvaluator\")))");
         if ( info.resolve(b) )
...
*/
         AbstractTask t;
         t = ZeusParser.primitiveTask(db,"(:Primitive MakeJOLoadForecast :time (1) :cost (0) :produced_facts ((:type JOLoadForecast :id newLoadForecast :modifiers 1 :attributes ((status ?var671)(name ?var432)(className ?var673)(expectedLoad ?var674)(toTime ?var440)(fromTime ?var442)))))");
         if ( t.resolve(b) )
            agent.addTask(t);
         stream = new FileInputStream("LoadForecasting.clp");
