...
public class Boerse {
   protected static void version() {
      System.err.println("ZeusAgent - Boerse version: 1.1");
      System.exit(0);
   }

   protected static void usage() {
      System.err.println("Usage: java Boerse -s <dns_file> -o <ontology_file> [-gui ViewerProg] [-e <ExternalProg>] [-r ResourceProg] [-name <AgentName>] [-debug] [-h] [-v]");
      System.exit(0);
   }
   ...
   try { 
      ...
      String name = new String ("Boerse");
      Bindings b = new Bindings("Boerse");
      ...
      for( int j = 0; j < arg.length; j++ ) {
         if ( arg[j].equals("-s") &&  ++j < arg.length )
            ...
            Core.debug = true;
            Core.setDebuggerOutputFile("Boerse.log");
         }
...
*/
         AbstractTask t;
         stream = new FileInputStream("Handelsplatz.clp");
         t = ZeusParser.reteKB(db,stream);
         stream.close();
         if ( t.resolve(b) )
            agent.addTask(t);
         t = ZeusParser.primitiveTask(db,"(:Primitive DummyForPX :time (1) :cost (0))");
         if ( t.resolve(b) )
            agent.addTask(t);
/*
...
*/
         Fact f1;
         f1 = ZeusParser.fact(db,"(:type AgentName :id Boersenname :modifiers 0 :attributes ((name PrismaXchange)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
