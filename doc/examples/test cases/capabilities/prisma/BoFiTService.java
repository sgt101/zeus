...
public class BoFiTService {
   protected static void version() {
      System.err.println("ZeusAgent - BoFiTService version: 1.1");
      System.exit(0);
   }

   protected static void usage() {
      System.err.println("Usage: java BoFiTService -s <dns_file> -o <ontology_file> [-gui ViewerProg] [-e <ExternalProg>] [-r ResourceProg] [-name <AgentName>] [-debug] [-h] [-v]");
      System.exit(0);
   }
   ...
   try { 
      ...
      String name = new String ("BoFiTService");
      Bindings b = new Bindings("BoFiTService");
      ...
      for( int j = 0; j < arg.length; j++ ) {
         if ( arg[j].equals("-s") &&  ++j < arg.length )
            ...
            Core.debug = true;
            Core.setDebuggerOutputFile("BoFiTService.log");
         }
      ...
      if ( dns_file == null ) {
         ...
         agent = new ZeusAgent(name,ontology_file,nameservers,7,20,true,false);
...
*/
         ProtocolInfo info;
         info = ZeusParser.protocolInfo(db,"(:name \"zeus.actors.graphs.ContractNetRespondent\" :type Respondent :constraints ((:fact (:type ZeusFact :id var425 :modifiers 1) :type 0 :strategy \"zeus.actors.graphs.DefaultRespondentEvaluator\")))");
         if ( info.resolve(b) )
...
*/
         AbstractTask t;
         t = ZeusParser.primitiveTask(db,"(:Primitive MakeJOMarginalCost :time (1) :cost (0) :produced_facts ((:type JOMarginalCostMatrix :id var629 :modifiers 1 :attributes ((status ?var630)(matrix ?var631)(name ?var632)(NumberOfRows ?var633)(className ?var634)(toTime ?var635)(isQuadratic ?var636)(fromTime ?var637)))))");
         if ( t.resolve(b) )
            agent.addTask(t);
/*
