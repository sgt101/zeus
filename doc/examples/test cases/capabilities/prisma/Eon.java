...
public class Eon {
   protected static void version() {
      System.err.println("ZeusAgent - Eon version: 1.1");
      System.exit(0);
   }

   protected static void usage() {
      System.err.println("Usage: java Eon -s <dns_file> -o <ontology_file> [-gui ViewerProg] [-e <ExternalProg>] [-r ResourceProg] [-name <AgentName>] [-debug] [-h] [-v]");
      System.exit(0);
   }
   ...
   try { 
      ...
      String name = new String ("Eon");
      Bindings b = new Bindings("Eon");
      ...
      for( int j = 0; j < arg.length; j++ ) {
         if ( arg[j].equals("-s") &&  ++j < arg.length )
            ...
            Core.debug = true;
            Core.setDebuggerOutputFile("Eon.log");
         }
...
*/
         ...
         item = ZeusParser.abilityDbItem(db,"(:agent RWE :ability (:fact (:type OTCContract :id var339 :modifiers 1 :attributes ((status ?var81)(name ?var340)(demandId ?var83)(price ?var341)(time ?var342)(quantity ?var343)(supplyId ?var87))) :time 0 :cost 0.0))");
         if ( item.resolve(b) )
            agent.addAbility(item);

         agent.addRelation("Veba","peer");
         item = ZeusParser.abilityDbItem(db,"(:agent Veba :ability (:fact (:type OTCContract :id var439 :modifiers 1 :attributes ((status ?var440)(name ?var441)(demandId ?var442)(price ?var443)(time ?var444)(quantity ?var445)(supplyId ?var446))) :time 0 :cost 0.0))");
         if ( item.resolve(b) )
            agent.addAbility(item);
