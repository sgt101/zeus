...
public class RWE {
   protected static void version() {
      System.err.println("ZeusAgent - RWE version: 1.1");
      System.exit(0);
   }

   protected static void usage() {
      System.err.println("Usage: java RWE -s <dns_file> -o <ontology_file> [-gui ViewerProg] [-e <ExternalProg>] [-r ResourceProg] [-name <AgentName>] [-debug] [-h] [-v]");
      System.exit(0);
   }
   ...
   try { 
      ...
      String name = new String ("RWE");
      Bindings b = new Bindings("RWE");
      ...
      for( int j = 0; j < arg.length; j++ ) {
         if ( arg[j].equals("-s") &&  ++j < arg.length )
            ...
            Core.debug = true;
            Core.setDebuggerOutputFile("RWE.log");
         }
...
*/
         ...
         item = ZeusParser.abilityDbItem(db,"(:agent Eon :ability (:fact (:type OTCContract :id var349 :modifiers 1 :attributes ((status ?var105)(name ?var350)(demandId ?var111)(price ?var351)(time ?var352)(quantity ?var353)(supplyId ?var115))) :time 0 :cost 0.0))");
         if ( item.resolve(b) )
            agent.addAbility(item);

         agent.addRelation("Veba","peer");
         item = ZeusParser.abilityDbItem(db,"(:agent Veba :ability (:fact (:type OTCContract :id var431 :modifiers 1 :attributes ((status ?var432)(name ?var433)(demandId ?var434)(price ?var435)(time ?var436)(quantity ?var437)(supplyId ?var438))) :time 0 :cost 0.0))");
         if ( item.resolve(b) )
            agent.addAbility(item);
