...
public class MonitorMaker {
   protected static void version() {
      System.err.println("ZeusAgent - MonitorMaker version: 1.2.2");
      System.exit(0);
   }

   protected static void usage() {
      System.err.println("Usage: java MonitorMaker -s <dns_file> -o <ontology_file> [-gui ViewerProg] [-e <ExternalProg>] [-r ResourceProg] [-name <AgentName>] [-debug] [-h] [-v]");
      System.exit(0);
   }
   ...
   try { 
      ...
      String name = new String ("MonitorMaker");
      Bindings b = new Bindings("MonitorMaker");
      ...
      for( int j = 0; j < arg.length; j++ ) {
         if ( arg[j].equals("-s") &&  ++j < arg.length )
            ...
            Core.debug = true;
            Core.setDebuggerOutputFile("MonitorMaker.log");
         }
      ...
      if ( dns_file == null ) {
         ...
         agent = new ZeusAgent(name,ontology_file,nameservers,2,25,true,false);
...
*/
         ProtocolInfo info;
         info = ZeusParser.protocolInfo(db,"(:name \"zeus.actors.graphs.ContractNetRespondent\" :type Respondent :constraints ((:fact (:type ZeusFact :id var268 :modifiers 1) :type 0 :strategy \"zeus.actors.graphs.LinearRespondentEvaluator\" :parameters (\"step.default\" \"2\" \"noquibble.range\" \"2\" \"max.percent\" \"125\" \"min.percent\" \"102\"))))");
         if ( info.resolve(b) )
...
*/
         AbstractTask t;
         t = ZeusParser.primitiveTask(db,"(:Primitive MakeMonitor :time ((1 * ?monitor.number)) :cost ((if (?monitor.type == sgva) then (120 * ?monitor.number) else (100 * ?monitor.number))) :consumed_facts ((:type CRT :id crt :modifiers 1 :attributes ((unit_cost ?var64)(number ?monitor.number)))(:type GFXadapter :id card :modifiers 1 :attributes ((unit_cost ?var67)(type ?monitor.type)(number ?monitor.number)))) :produced_facts ((:type Monitor :id monitor :modifiers 1 :attributes ((unit_cost ?var71)(type ?var316)(number ?var315)))))");
         if ( t.resolve(b) )
            agent.addTask(t);
/*
...
*/
         Fact f1;
         f1 = ZeusParser.fact(db,"(:type CRT :id crt :modifiers 0 :attributes ((unit_cost 60)(number 2000)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
         f1 = ZeusParser.fact(db,"(:type GFXadapter :id vga_card :modifiers 0 :attributes ((unit_cost 10)(type vga)(number 2000)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
         f1 = ZeusParser.fact(db,"(:type GFXadapter :id svga_card :modifiers 0 :attributes ((unit_cost 10)(type svga)(number 2000)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
