...
public class South {
   protected static void version() {
      System.err.println("ZeusAgent - South version: 1.02");
      System.exit(0);
   }

   protected static void usage() {
      System.err.println("Usage: java South -s <dns_file> -o <ontology_file> [-gui ViewerProg] [-e <ExternalProg>] [-r ResourceProg] [-debug] [-h] [-v]");
      System.exit(0);
   }
   ...
   public static void main(String[] arg) {
      ...
      for( int j = 0; j < arg.length; j++ ) {
         if ( arg[j].equals("-s") &&  ++j < arg.length )
            ...
            Core.debug = true;
            Core.setDebuggerOutputFile("South.log");
         }
      ...
      try {
         ...
         agent = new ZeusAgent("South",ontology_file,nameservers,2,30,true,true);
...
*/
         ...
         if ( t.resolve(b) )
            agent.addTask(t);
         t = ZeusParser.primitiveTask(db,"(:Primitive Type2Repair :time (1) :cost (25) :consumed_facts ((:type Engineer :id engineer :modifiers 25 :attributes ((skill >=?repair.type)(unit_cost ?var56)(number 1)))) :produced_facts ((:type Repair :id repair :modifiers 1 :attributes ((type 2)(fault ?var52)(id ?var53)))))");
         if ( t.resolve(b) )
            agent.addTask(t);
