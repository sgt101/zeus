...
*/
         ProtocolInfo info;
         info = ZeusParser.protocolInfo(db,"(:name \"zeus.actors.graphs.ContractNetInitiator\" :type Initiator :constraints ((:fact (:type ZeusFact :id var5 :modifiers 1) :type 0 :strategy \"zeus.actors.graphs.DefaultInitiatorEvaluator\")))");
         if ( info.resolve(b) )
...
*/
         AbstractTask t;
         t = ZeusParser.primitiveTask(db,"(:Primitive Task2 :time (1) :cost (0))");
         if ( t.resolve(b) )
            agent.addTask(t);
/*
...
*/
         AbilityDbItem item;
         agent.addRelation("Agent5","superior");
         item = ZeusParser.abilityDbItem(db,"(:agent Agent5 :ability (:fact (:type Entity :id var2 :modifiers 1 :attributes ((unit_cost ?var3)(number ?var4))) :time 0 :cost 0.0))");
         if ( item.resolve(b) )
...
*/
         Fact f1;
         f1 = ZeusParser.fact(db,"(:type Entity :id fact1 :modifiers 0 :attributes ((unit_cost 0)(number 1)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
