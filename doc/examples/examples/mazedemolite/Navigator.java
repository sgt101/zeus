...
public class Navigator {
   protected static void version() {
      System.err.println("ZeusAgent - Navigator version: 1.04");
      System.exit(0);
   }

   protected static void usage() {
      System.err.println("Usage: java Navigator -s <dns_file> -o <ontology_file> [-gui ViewerProg] [-e <ExternalProg>] [-r ResourceProg] [-name <AgentName>] [-debug] [-h] [-v]");
      System.exit(0);
   }
   ...
   try { 
      ...
      String name = new String ("Navigator");
      Bindings b = new Bindings("Navigator");
      ...
      for( int j = 0; j < arg.length; j++ ) {
         if ( arg[j].equals("-s") &&  ++j < arg.length )
            ...
            Core.debug = true;
            Core.setDebuggerOutputFile("Navigator.log");
         }
      ...
      if ( dns_file == null ) {
         ...
         agent = new ZeusAgent(name,ontology_file,nameservers,2,20,false,true);
...
*/
         AbstractTask t;
         stream = new FileInputStream("Register.clp");
         t = ZeusParser.reteKB(db,stream);
         stream.close();
         if ( t.resolve(b) )
            agent.addTask(t);
         stream = new FileInputStream("Navigate.clp");
         t = ZeusParser.reteKB(db,stream);
         stream.close();
         if ( t.resolve(b) )
            agent.addTask(t);
...
*/
         AbilityDbItem item;
         agent.addRelation("Environment","peer");

/*
         Initialising ResourceDb
*/
         Fact f1;
         f1 = ZeusParser.fact(db,"(:type thisMove :id fact39 :modifiers 0 :attributes ((west false)(south false)(east false)(north false)(id Navigator)))");
         if ( f1.resolve(b) )
            agent.addFact(f1);
