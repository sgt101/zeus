...
public class CartridgeCorp {
   protected static void version() {
      System.err.println("ZeusAgent - CartridgeCorp version: 1.1");
      System.exit(0);
   }

   protected static void usage() {
      System.err.println("Usage: java CartridgeCorp -s <dns_file> -o <ontology_file> [-gui ViewerProg] [-e <ExternalProg>] [-r ResourceProg] [-name <AgentName>] [-debug] [-h] [-v]");
      System.exit(0);
   }
   ...
   try { 
      ...
      String name = new String ("CartridgeCorp");
      Bindings b = new Bindings("CartridgeCorp");
      ...
      for( int j = 0; j < arg.length; j++ ) {
         if ( arg[j].equals("-s") &&  ++j < arg.length )
            ...
            Core.debug = true;
            Core.setDebuggerOutputFile("CartridgeCorp.log");
         }
      ...
      if ( dns_file == null ) {
         ...
         agent = new ZeusAgent(name,ontology_file,nameservers,1,25,true,false);
...
*/
         ProtocolInfo info;
         info = ZeusParser.protocolInfo(db,"(:name \"zeus.actors.graphs.ContractNetRespondent\" :type Respondent :constraints ((:fact (:type ZeusFact :id var104 :modifiers 1) :type 0 :strategy \"zeus.actors.graphs.DefaultRespondentEvaluator\" :relations (superior)) (:fact (:type ZeusFact :id var112 :modifiers 1) :type 0 :strategy \"zeus.actors.graphs.LinearRespondentEvaluator\" :parameters (\"step.default\" \"2\" \"noquibble.range\" \"2\" \"max.percent\" \"120\" \"min.percent\" \"105\"))))");
         if ( info.resolve(b) )
...
*/
         AbilityDbItem item;
         agent.addRelation("PrinterMaker","superior");
