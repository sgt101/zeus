...
public class InfoSupplier {
   protected static void version() {
      System.err.println("ZeusAgent - InfoSupplier version: 1.04");
      System.exit(0);
   }

   protected static void usage() {
      System.err.println("Usage: java InfoSupplier -s <dns_file> -o <ontology_file> [-gui ViewerProg] [-e <ExternalProg>] [-r ResourceProg] [-name <AgentName>] [-debug] [-h] [-v]");
      System.exit(0);
   }
   ...
   try { 
      ...
      String name = new String ("InfoSupplier");
      Bindings b = new Bindings("InfoSupplier");
      ...
      for( int j = 0; j < arg.length; j++ ) {
         if ( arg[j].equals("-s") &&  ++j < arg.length )
            ...
            Core.debug = true;
            Core.setDebuggerOutputFile("InfoSupplier.log");
         }
...
*/
         ProtocolInfo info;
         info = ZeusParser.protocolInfo(db,"(:name \"zeus.actors.graphs.ContractNetRespondent\" :type Respondent :constraints ((:fact (:type ZeusFact :id var73 :modifiers 1) :type 0 :strategy \"zeus.actors.graphs.DefaultRespondentEvaluator\")))");
         if ( info.resolve(b) )
...
*/
         AbstractTask t;
         t = ZeusParser.primitiveTask(db,"(:Primitive SupplyInfo :time (1) :cost (0) :produced_facts ((:type output :id var80 :modifiers 1 :attributes ((info \"Hello World\")))))");
         if ( t.resolve(b) )
            agent.addTask(t);
/*
