...
public class Displayer {
   protected static void version() {
      System.err.println("ZeusAgent - Displayer version: 1.04");
      System.exit(0);
   }

   protected static void usage() {
      System.err.println("Usage: java Displayer -s <dns_file> -o <ontology_file> [-gui ViewerProg] [-e <ExternalProg>] [-r ResourceProg] [-name <AgentName>] [-debug] [-h] [-v]");
      System.exit(0);
   }
   ...
   try { 
      ...
      String name = new String ("Displayer");
      Bindings b = new Bindings("Displayer");
      ...
      for( int j = 0; j < arg.length; j++ ) {
         if ( arg[j].equals("-s") &&  ++j < arg.length )
            ...
            Core.debug = true;
            Core.setDebuggerOutputFile("Displayer.log");
         }
...
*/
         ProtocolInfo info;
         info = ZeusParser.protocolInfo(db,"(:name \"zeus.actors.graphs.ContractNetInitiator\" :type Initiator :constraints ((:fact (:type ZeusFact :id var74 :modifiers 1) :type 0 :strategy \"zeus.actors.graphs.DefaultInitiatorEvaluator\")))");
         if ( info.resolve(b) )
...
*/
         AbstractTask t;
         t = ZeusParser.primitiveTask(db,"(:Primitive DisplayInfo :time (1) :cost (0) :consumed_facts ((:type output :id var75 :modifiers 1 :attributes ((info ?var76)))) :produced_facts ((:type displayed :id var77 :modifiers 1 :attributes ((flag true)))))");
         if ( t.resolve(b) )
            agent.addTask(t);
/*
