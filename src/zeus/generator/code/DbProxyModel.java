...
* DbProxyModel.java
*
* The underlying model for the DbProxy Table
*****************************************************************************/
...
public class DbProxyModel extends UtilityModel {
  ...
  protected static final String[] columnNames = {
     "Name", "Host", "DNS file", "Proxy classpath",
  ...
  public DbProxyModel(GenerationPlan genplan) {
  ...
  public void addNewRow() {
     genplan.createDbProxy();
  }
  public void removeRows(int[] rows) {
     for(int i = 0; i < rows.length; i++ ) {
        DbProxyInfo info = (DbProxyInfo)data.elementAt(rows[i]-i);
        genplan.removeDbProxy(info.id);
     }
  }

  protected void refresh()  {
    data.removeAllElements();
    DbProxyInfo[] info = genplan.getDbProxys();
  ...
  public Object getValueAt(int row, int column) {
     DbProxyInfo info = (DbProxyInfo)data.elementAt(row);
     switch(column) {
        case NAME:
             return info.name;
  ...
  public void setValueAt(Object aValue, int row, int column) {
	...
	case PATH:
             info.path = updateString(info.path,aValue);
             break;
     ...
     if ( changed ) genplan.setDbProxy(info);
  }
}
