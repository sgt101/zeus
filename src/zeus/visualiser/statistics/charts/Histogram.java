...
public class Histogram implements DrawObject {
   ...
   public Histogram(double[] values, String[] labels, String title) {
      setData(values,labels,title);
   }

   public Histogram(double[][] values, String[] labels, String[] keys,
                    String title) {
   ...
   public void drawYourSelf(Graphics g) {
      ...
      switch( type ) {
         case TYPE1:
            ...
            for( int i = 0; i < bars.size(); i++ ) {
               bar = (Bar) bars.elementAt(i);
               x1 = x0 + (mw+gap)*i + gap/2;
               x2 = x1 + (mw+gap) - gap/2;
               y2 = (int)(y0 - (bar.value/max)*(y0-3.0*h));
               g.setColor(bar.color);
               g.fillRect(Math.min(x1,x2),Math.min(y1,y2),
                          Math.abs(x2-x1),Math.abs(y2-y1));

               w = fm.stringWidth(bar.label);
               yp = bar.value > 0.0 ? y1 + h : y1 - h;
               g.setColor(Color.black);
               g.drawString(bar.label,x1+(mw+gap-w)/2,yp);
            }
            break;
         case TYPE2:
            g.drawLine(LEFT,3*h,LEFT,y-2*h);
            ...
            for( int i = 0; i < bars.size(); i++ ) {
               ...
               for( int j = 0; j < bar.values.length; j++ ) {
                  yval += bar.values[j];
                  y1 = y2;
                  y2 = (int)(y0 - (yval/max)*(y0-3.0*h));
                  g.setColor(ColorManager.getColor(j));
                  g.fillRect(Math.min(x1,x2),Math.min(y1,y2),
                             Math.abs(x2-x1),Math.abs(y2-y1));
               }
               w = fm.stringWidth(bar.label);
               yp = yval > 0.0 ? y0 + h : y0 - h;
               g.setColor(Color.black);
               g.drawString(bar.label,x1+(mw+gap-w)/2,yp);
            }
            break;
      ...
      xp = x0; yp = y0;
      while( v-TINY <= max ) {
         String yval = Double.toString(v);
         yval = Misc.decimalPlaces(yval,2);
