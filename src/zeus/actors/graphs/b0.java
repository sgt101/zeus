...
public class b0 extends Node {
    
   // ST 050500 1.03bB node description due to CVB
   private String node_desc = "do for each external goal";
   ...
   protected int exec() {
      ...
      Core.DEBUG(2,"b0 Input gs = " + gs);

      // Now create split node for next_lot
      data = new DStruct[gs.goal.size()];
      for(int i = 0; i < data.length; i++ ) {
         data[i] = new DStruct();
         data[i].goal = new Vector();
         data[i].goal.addElement(gs.goal.elementAt(i));
